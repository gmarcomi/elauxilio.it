...
# controllo sessione
if ($sessionName ne ""){
    $presenter= new PresenterAnswers("gnome-3.22-ecco-nuovo-control-center.tmpl",1,"none",$sessionName,0);
    $logged=1;
    $admin = $user->isAdmin();
}
else{
    $presenter= new PresenterAnswers("gnome-3.22-ecco-nuovo-control-center.tmpl",0,"none");
}
...
$view->param(id => "gnome-3.22-ecco-nuovo-control-center");
my $answers = new Answers();
$view->param(username => $sessionName);
if ($get{'all'} eq "true") {
  $view->param(answers => $answers->getAnswers($admin,"gnome-3.22-ecco-nuovo-control-center"));
  $view->param(total => 1);
}
else{
  $view->param(answers => $answers->getFirstAnswers($admin,"gnome-3.22-ecco-nuovo-control-center"));
  $view->param(total => 0);
}
