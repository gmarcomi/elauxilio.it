...
# controllo sessione
if ($sessionName ne ""){
    $presenter= new PresenterAnswers("linux-Mint-rafforza-la-sicurezza.tmpl",1,"none",$sessionName,0);
    $logged=1;
    $admin = $user->isAdmin();
}
else{
    $presenter= new PresenterAnswers("linux-Mint-rafforza-la-sicurezza.tmpl",0,"none");
}
...
$view->param(id => "linux-Mint-rafforza-la-sicurezza");
my $answers = new Answers();
$view->param(username => $sessionName);
if ($get{'all'} eq "true") {
  $view->param(answers => $answers->getAnswers($admin,"linux-Mint-rafforza-la-sicurezza"));
  $view->param(total => 1);
}
else{
  $view->param(answers => $answers->getFirstAnswers($admin,"linux-Mint-rafforza-la-sicurezza"));
  $view->param(total => 0);
}
