...
# controllo sessione
if ($sessionName ne ""){
    $presenter= new PresenterAnswers("stylelint-a-caccia-errori-css.tmpl",1,"none",$sessionName,0);
    $logged=1;
    $admin = $user->isAdmin();
}
else{
    $presenter= new PresenterAnswers("stylelint-a-caccia-errori-css.tmpl",0,"none");
}
...
$view->param(id => "stylelint-a-caccia-errori-css");
my $answers = new Answers();
$view->param(username => $sessionName);
if ($get{'all'} eq "true") {
  $view->param(answers => $answers->getAnswers($admin,"stylelint-a-caccia-errori-css"));
  $view->param(total => 1);
}
else{
  $view->param(answers => $answers->getFirstAnswers($admin,"stylelint-a-caccia-errori-css"));
  $view->param(total => 0);
}
