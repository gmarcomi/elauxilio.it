...
# controllo sessione
if ($sessionName ne ""){
    $presenter= new PresenterAnswers("fedy-cambia-dominio.tmpl",1,"none",$sessionName,0);
    $logged=1;
    $admin = $user->isAdmin();
}
else{
    $presenter= new PresenterAnswers("fedy-cambia-dominio.tmpl",0,"none");
}
...
$view->param(id => "fedy-cambia-dominio");
my $answers = new Answers();
$view->param(username => $sessionName);
if ($get{'all'} eq "true") {
  $view->param(answers => $answers->getAnswers($admin,"fedy-cambia-dominio"));
  $view->param(total => 1);
}
else{
  $view->param(answers => $answers->getFirstAnswers($admin,"fedy-cambia-dominio"));
  $view->param(total => 0);
}
