...
# controllo sessione
if ($sessionName ne ""){
    $presenter= new PresenterAnswers("ubuntu-allo-stand-nokia-del-mwc-2016.tmpl",1,"none",$sessionName,0);
    $logged=1;
    $admin = $user->isAdmin();
}
else{
    $presenter= new PresenterAnswers("ubuntu-allo-stand-nokia-del-mwc-2016.tmpl",0,"none");
}
...
$view->param(id => "ubuntu-allo-stand-nokia-del-mwc-2016");
my $answers = new Answers();
$view->param(username => $sessionName);
if ($get{'all'} eq "true") {
  $view->param(answers => $answers->getAnswers($admin,"ubuntu-allo-stand-nokia-del-mwc-2016"));
  $view->param(total => 1);
}
else{
  $view->param(answers => $answers->getFirstAnswers($admin,"ubuntu-allo-stand-nokia-del-mwc-2016"));
  $view->param(total => 0);
}
